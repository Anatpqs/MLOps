...
def train_model(
    ...
    X_train = X_train.reshape(X_train.shape[0], X_train.shape[1], 1)  # Ajouter une dimension pour features
    ...
    # Entraîner le modèle
    model.fit(X_train, y_train, batch_size=int(params['batch_size']), epochs=int(params['epochs']), verbose=0)
    return model
